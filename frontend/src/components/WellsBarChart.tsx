...
export default function WellsBarChart({ wells }: { wells: WellSummary[] }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-lg p-4 shadow-sm">
      <h3 className="text-sm font-semibold text-ink mb-3">Key Metrics by Well (%)</h3>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
          <CartesianGrid stroke={colors.gridLine} vertical={false} />
          <XAxis dataKey="name" tick={{ fill: colors.inkMuted, fontSize: 12 }} axisLine={{ stroke: colors.borderStrong }} tickLine={false} />
          <YAxis tick={{ fill: colors.inkMuted, fontSize: 12 }} axisLine={{ stroke: colors.borderStrong }} tickLine={false} />
          <Tooltip
            contentStyle={{
          ...
          <Bar dataKey="Avg VSH" fill={colors.reservoir} radius={[4, 4, 0, 0]} />
          <Bar dataKey="Avg PHIE" fill={colors.accent} radius={[4, 4, 0, 0]} />
          <Bar dataKey="Avg SWE" fill={colors.nonReservoir} radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );

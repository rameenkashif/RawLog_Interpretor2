...
class TestBlindWellSectionMapsEndpoint:
    def test_ok(self, client, monkeypatch):
        png_bytes = b"\x89PNG\r\n\x1a\nfake"
        monkeypatch.setattr(bwp, "render_blind_well_section_maps", lambda blind_well_id="Z-02_RAW": png_bytes)
        resp = client.get("/api/prediction/blind-well/section-maps")
        assert resp.status_code == 200
        assert resp.headers["content-type"] == "image/png"
        assert resp.content == png_bytes
    ...
    def test_accepts_blind_well_id_query_param(self, client, monkeypatch):
        ...
        monkeypatch.setattr(bwp, "render_blind_well_section_maps", _fake)
        resp = client.get("/api/prediction/blind-well/section-maps", params={"blind_well_id": "Z-05_RAW"})
        assert resp.status_code == 200
        assert seen["blind_well_id"] == "Z-05_RAW"
    ...
    def test_unknown_well_is_404(self, client, monkeypatch):
        ...
        monkeypatch.setattr(bwp, "render_blind_well_section_maps", _fake)
        resp = client.get("/api/prediction/blind-well/section-maps", params={"blind_well_id": "DOES_NOT_EXIST"})
        assert resp.status_code == 404
